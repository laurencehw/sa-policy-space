...
function GapBar({ allocated, recommended }: { allocated: number; recommended: number }) {
  const pct = Math.min((allocated / recommended) * 100, 100);
  const isOver = allocated > recommended;
  return (
    <div className="w-full bg-gray-100 rounded-full h-2.5 overflow-hidden">
      <div
        className={`h-2.5 rounded-full transition-all ${isOver ? "bg-blue-400" : pct >= 80 ? "bg-green-400" : pct >= 50 ? "bg-amber-400" : "bg-red-400"}`}
...
export default function BudgetPage() {
  const packages = budgetData.packages as PackageBudget[];
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1 max-w-2xl">
          ...
          Source: {meta.base_year} MTBPS and Parliamentary Committee BRRRs.
        </p>
      </div>
          ...
          {packages.map((pkg) => {
            ...
            return (
              <div key={pkg.package_id}>
                ...
                {/* Dual bar: allocated (solid) over recommended (outline) */}
